...
```{r gen-data}
qc_gen_summary <- function(year_begin, year_end, top = 10L) {
  
  qc[year >= year_begin & year <= year_end, .(
    nb = sum(nb)
  ), .(sex, group_label)][, .(
    group_label = group_label,
    nb = nb,
    rnk = rank(-nb, ties.method = "min")
  ), .(sex)][rnk <= top]
  
}

qc_gen_data <- function(year_begin, year_end, top = 10L) {
  ...
  # A FINALISER
  qc[name == group_name][gen_summary, flag := TRUE, on = .(sex, group_label)][flag == TRUE][, flag := NULL]
  
}

qc_gen_data(1981L, 1996L)

qc_gen_data <- qc[name == group_name][unique(rbindlist(list(
  qc_gen_summary(1981L, 1996L),
  qc_gen_summary(1997L, 2012L),
  qc_gen_summary(2013L, 2028L)
))[, .(sex, group_label)]), flag := TRUE, on = .(sex, group_label)][flag == TRUE][, flag := NULL]

gen_limits <- qc_gen_data[, c(0, max(group_nb))]
```
...
## La génération Alpha (2013 - 2021)

---

## Homonymes tolérables

```{r namesake-bearable}
namesake_bearable_1 <- qc[sex == "M" & group_name == "VICTOR"]
namesake_bearable_2 <- qc[sex == "F" & group_name == "KIM"]

limits_namesake_bearable <- rbindlist(list(namesake_bearable_1, namesake_bearable_2))[, c(0, max(nb))]

namesake_bearable_1_plot <- ggplot(
  data = namesake_bearable_1,
  mapping = aes(
    x = year,
    y = nb,
  ...
  geom_line() +
  scale_x_continuous(name = NULL) +
  scale_y_continuous(name = "Nombre de naissances", limits = limits_namesake_bearable) +
  scale_color_discrete(name = NULL)

namesake_bearable_2_plot <- ggplot(
  data = namesake_bearable_2,
  mapping = aes(
    x = year,
    y = nb,
    color = reorder(name, -total_nb)
  ...
  geom_line() +
  scale_x_continuous(name = NULL) +
  scale_y_continuous(name = NULL, labels = NULL, limits = limits_namesake_bearable) +
  scale_color_discrete(name = NULL)

namesake_bearable_1_plot + namesake_bearable_2_plot
```
...
```{r namesake-painful}
namesake_painful_1 <- qc[sex == "M" & group_name == "HAYDEN"]
namesake_painful_2 <- qc[sex == "F" & group_name == "CAROLANE"]

limits_namesake_painful <- rbindlist(list(namesake_painful_1, namesake_painful_2))[, c(0, max(nb))]

namesake_painful_1_plot <- ggplot(
  data = namesake_painful_1,
  mapping = aes(
    x = year,
    y = nb,
    color = reorder(name, -total_nb)
  ...
  geom_line() +
  scale_x_continuous(name = NULL) +
  scale_y_continuous(name = "Nombre de naissances", limits = limits_namesake_painful) +
  scale_color_discrete(name = NULL)

namesake_painful_2_plot <- ggplot(
  data = namesake_painful_2,
  mapping = aes(
    x = year,
    y = nb,
    color = reorder(name, -total_nb)
  ...
  geom_line() +
  scale_x_continuous(name = NULL) +
  scale_y_continuous(name = NULL, labels = NULL, limits = limits_namesake_painful) +
  scale_color_discrete(name = NULL)

namesake_painful_1_plot + namesake_painful_2_plot
```
